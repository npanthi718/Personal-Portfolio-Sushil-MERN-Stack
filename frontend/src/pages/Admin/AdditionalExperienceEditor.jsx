...
export default function AdditionalExperienceEditor({ setSnack }) {
    ...
    async function saveAll(next) {
        try {
            ...
            if (res.ok) {
                setItems(next);
                setSnack({ open: true, message: 'Additional experience saved!', severity: 'success' });
            } else {
                setSnack({ open: true, message: 'Failed to save experience', severity: 'error' });
            }
        } catch (err) {
    ...
    return (
        <Grid container spacing={4}>
            ...
            <Grid size={12}>
                <Divider sx={{ my: 4 }} />
                <Typography variant="h6" gutterBottom color="primary">Manage Additional Experience</Typography>
                
                <Paper variant="outlined" sx={{ p: 3, mb: 4, backgroundColor: 'transparent', border: '1px solid #444' }}>
                    <Typography variant="subtitle1" sx={{ mb: 2, fontWeight: 'bold' }}>Add New Experience</Typography>
                    <form onSubmit={add}>
